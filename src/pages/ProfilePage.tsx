...
export default function ProfilePage({ session }: { session: Session }) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-20 px-6 space-y-24 animate-in fade-in duration-1000">
      ...
      {/* СЕКЦИЯ: ВХОДЯЩИЕ ЗАПРОСЫ (УПРАВЛЕНИЕ) */}
      <section className="space-y-12">
        <header className="flex justify-between items-end border-b border-slate-100 pb-8">
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-sm font-medium">Управление доступом к вашим объектам</p>
          </div>
            ...
            {incomingRequests.map(req => (
              <div key={req.id} className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-[0_10px_40px_rgba(0,0,0,0.02)] space-y-8 group transition-all hover:border-indigo-100">
                <div className="space-y-1">
                  ...
                  <h4 className="text-xl font-bold text-slate-900 tracking-tight leading-none">{req.item?.title}</h4>
                </div>

                <div className="space-y-1">
                  <p className="text-[9px] font-bold text-slate-300 uppercase tracking-[0.3em]">Заявитель</p>
                  <p className="text-sm font-bold text-slate-800 tracking-tight">{req.borrower?.full_name}</p>
                  <p className="text-xs text-slate-400 font-medium">{req.borrower?.email}</p>
                </div>
                
                <div className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Период</span>
...
{req.status === 'pending' ? (
  <div className="flex gap-3">
    <button 
      ...
      className="flex-1 bg-slate-900 text-white py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-indigo-600 transition-all active:scale-95"
    >
      Принять
    </button>
    <button 
      onClick={() => updateBookingStatus(req.id, 'cancelled')}
      className="flex-1 bg-white border border-slate-100 text-slate-400 py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-red-50 hover:text-red-500 transition-all"
    >
      Отказ
    </button>
  </div>
) : (
  <div className={`text-center py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border ${
    req.status === 'confirmed' 
      ? 'bg-indigo-50 text-indigo-600 border-indigo-100' 
      : 'bg-red-50 text-red-500 border-red-100'
  }`}>
    {req.status === 'confirmed' ? 'Статус: Подтверждено' : 'Статус: Отклонено'}
  </div>
)}
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
        ...
        <section className="space-y-10">
          ...
          <div className="space-y-4">
            {myItems.length === 0 ? (
              <p className="text-slate-300 text-xs italic ml-2">Список ваших объектов пуст.</p>
            ) : (
              myItems.map(item => (
                <div key={item.id} className="bg-white p-6 rounded-[28px] border border-slate-100 flex justify-between items-center group hover:bg-slate-50 transition-all">
                  <div className="space-y-1">
                    <span className="font-bold text-slate-800 tracking-tight block">{item.title}</span>
                    <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">Доступен к аренде</span>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black text-slate-900 block">{item.price_per_day} грн</span>
        ...
        <section className="space-y-10">
          ...
          <div className="space-y-4">
            {myBookings.length === 0 ? (
              <p className="text-slate-300 text-xs italic ml-2">Активные бронирования не найдены.</p>
            ) : (
              myBookings.map(b => (
                <div key={b.id} className="bg-white p-6 rounded-[28px] border border-slate-100 flex justify-between items-center">
                  <div className="space-y-1">
                    <p className="font-bold text-slate-800 tracking-tight block leading-none mb-1">{b.item?.title}</p>
                    <p className="text-[9px] font-bold text-slate-400 tracking-tight uppercase">
                      {new Date(b.start_date).toLocaleDateString()} — {new Date(b.end_date).toLocaleDateString()}
                    </p>
                  </div>
<div className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.1em] border ${
